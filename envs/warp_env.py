...
class WarpEnv:
    ...
    def __init__(self, num_envs, num_obs, num_act, episode_length, MM_caching_frequency=1, seed=0, no_grad=True,
                 render=False, device='cuda'):
        self.seed = seed
    ...
    def reset_with_state(self, init_joint_q, init_joint_qd, env_ids=None, force_reset=True):
        ...
        if env_ids is not None:
            ...
            self.state.joint_q.view(self.num_envs, -1)[env_ids, :] = init_joint_q.view(-1, self.num_joint_q)[env_ids,
                                                                     :].clone()
            self.state.joint_qd.view(self.num_envs, -1)[env_ids, :] = init_joint_qd.view(-1, self.num_joint_qd)[env_ids,
                                                                      :].clone()
