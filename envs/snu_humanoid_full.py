...
class SNUHumanoidFullEnv(DFlexEnv):
    ...
    def __init__(self, render=False, device='cuda:0', num_envs=4096, seed=0, episode_length=1000, no_grad=True, stochastic_init=False, MM_caching_frequency = 1):
        ...
        self.use_full_humanoid = False
        self.mtu_actuations = False
        ...
        super(SNUHumanoidFullEnv, self).__init__(num_envs, num_obs, num_act, episode_length, MM_caching_frequency, seed, no_grad, render, device)
                    ...
                    if self.mtu_actuations:
                        for m in range(len(s.muscles)):
                            ...
                            self.renderer.add_line_strip(points, name=s.muscles[m].name + str(skel_index),
                                                         radius=0.0075, color=(
                                self.model.muscle_activation[muscle_start + m] / self.muscle_strengths[m], 0.2, 0.5),
    ...
    def calculateReward(self):
        up_reward = 0.1 * self.obs_buf[:, self.num_obs - 2] # 51
        heading_reward = self.obs_buf[:, self.num_obs - 1] # 52
