...
def load_bvh(
    ...
    for i in range(len(valid_indices)):
        joint_q_mask[
            0,
            model.joint_q_start[valid_indices[i]] : model.joint_q_start[
                valid_indices[i] + 1
            ],
        ] = 1.0
        ...
        for bvh_name in bvh_names:
            ...
            for channel_index, channel in enumerate(bvh_channel_map[bvh_name]):
                if channel == "Xposition":
                    ...
                    quat = df.quat_multiply(
                        quat,
                        df.quat_from_axis_angle(
                            np.array((1.0, 0.0, 0.0)),
                            motion_data[motion_index + channel_index] * math.pi / 180.0,
                        ),
                    )
            ...
            if bvh_name in bvh_link_map:
                ...
                joint_xforms[frame, bvh_index, :] = torch.tensor(
                    np.concatenate((pos * pos_scale, quat))
                )
    ...
    for bvh_name in bvh_names:
        if bvh_name in bvh_link_map:
            ...
            if type == df.JOINT_FREE:
                ...
                projected_vec = torch.nn.functional.normalize(
                    ref_forward_vec
                    - torch.sum(ref_forward_vec * ref_axis, dim=-1, keepdim=True)
                    * ref_axis,
                    dim=-1,
                )  # (num_frames, 3)
                ...
                projected_ref = torch.nn.functional.normalize(
                    ref_vec
                    - torch.sum(ref_vec * ref_axis, dim=-1, keepdim=True) * ref_axis,
                    dim=-1,
                )
                # use atan2 to get signed angle
                joint_angle = torch.atan2(
                    torch.sum(
                        torch.cross(projected_ref, projected_vec, dim=-1) * ref_axis,
                        dim=-1,
                    ),
