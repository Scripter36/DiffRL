...
def main():
    ...
    if choice == "1":
        ...
        env_pascal_case = snake_to_pascal(selected_env_name)
        log_dir = f"examples/logs/{env_pascal_case}/shac"
        ...
        env_snake_case = pascal_to_snake(selected_env_name)

        print(f"\033[93mRunning rendering for environment: {env_snake_case}\033[0m")  # Yellow text for status messages
        # Print command in white (default)
        print(f"python examples/train_shac.py --cfg examples/cfg/shac/{env_snake_case}.yaml --checkpoint {selected_policy} --play --render")
        os.system(
            f"python examples/train_shac.py --cfg examples/cfg/shac/{env_snake_case}.yaml --checkpoint {selected_policy} --play --render")
